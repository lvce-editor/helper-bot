...
test('adds repository link to root extension.json when missing', async () => {
  ...
  const mockFs = createMockFs({
    files: {
      [resolveUri('extension.json', clonedRepoUri)]: JSON.stringify(extensionJson, null, 2) + '\n',
    },
  })
...
test('adds repository link to monorepo extension.json when missing', async () => {
  ...
  const mockFs = createMockFs({
    files: {
      [resolveUri('packages/extension/extension.json', clonedRepoUri)]: JSON.stringify(extensionJson, null, 2) + '\n',
    },
  })
...
test('adds repository link to both root and monorepo extension.json when both exist and missing repository', async () => {
  ...
  const mockFs = createMockFs({
    files: {
      [resolveUri('extension.json', clonedRepoUri)]: JSON.stringify(rootExtensionJson, null, 2) + '\n',
      [resolveUri('packages/extension/extension.json', clonedRepoUri)]: JSON.stringify(monorepoExtensionJson, null, 2) + '\n',
    },
  })
...
test('handles invalid JSON gracefully', async () => {
  ...
  const mockFs = createMockFs({
    files: {
      [resolveUri('extension.json', clonedRepoUri)]: 'invalid json{',
    },
  })
